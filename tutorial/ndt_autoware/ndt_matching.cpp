...
int main(int argc, char** argv)
{
  ...
  //这样生成的节点句柄实例，如果launch文件中没有指定ns，那么默认是全局命名空间的，
  ros::NodeHandle nh;
  ...
  private_nh.getParam("method_type", method_type_tmp);// 0
  _method_type = static_cast<MethodType>(method_type_tmp); //将数字0123强制转化为对应的枚举类型
  private_nh.getParam("use_gnss", _use_gnss);// 1
  private_nh.getParam("queue_size", _queue_size); // 1
  private_nh.getParam("offset", _offset);// linear
  private_nh.getParam("get_height", _get_height); //false
  private_nh.getParam("use_local_transform", _use_local_transform);//false
  private_nh.getParam("use_imu", _use_imu);//false
  private_nh.getParam("use_odom", _use_odom);//false
  private_nh.getParam("imu_upside_down", _imu_upside_down);//false
  private_nh.getParam("imu_topic", _imu_topic);// /imu_raw

  if (nh.getParam("localizer", _localizer) == false)  //velodyne
  {
  ...
  estimate_twist_pub = nh.advertise<geometry_msgs::TwistStamped>("/estimate_twist", 10); //twist消息内容中包含了线速度和角速度
