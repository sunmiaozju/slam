...
static void imu_calc(ros::Time current_time) {
    ...
    //由于有旋转角度的存在，所以imu局部坐标系的xyz会偏一点，因此求取真实xyz方向的加速度需要考虑旋转
...
static void imu_callback(const sensor_msgs::Imu::Ptr &input) {
    ...
    //获取imu传来信息中的欧拉角度，imu中有一个四元数变量包含了这些信息
    ...
    //计算相邻两个imu帧的欧拉角差值，并且差值的范围在（-pi~pi）
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr &input) {
    ...
    for (pcl::PointCloud<pcl::PointXYZI>::const_iterator item = tmp.begin(); item != tmp.end(); item++) {
        ...
        //求该点到原点的向量模长，如果在设置的扫描范围（5m-200m）环里面，就保留下来
    ...
    //又新产生了一个指向全局map的共享指针，原来对map的修改可以继续使用这个共享指针来继续修改
    pcl::PointCloud<pcl::PointXYZI>::Ptr map_ptr(new pcl::PointCloud<pcl::PointXYZI>(map));
...
#endif
    //静态局部变量，下次调用这个函数的时候就不会在此定义了，而且保留之前的数值
    ...
    if (_method_type == MethodType::PCL_GENERIC) {
        ...
        t_localizer = ndt.getFinalTransformation();  //这个矩阵是当前帧配准到map上的转换矩阵
...
#endif
    //t_base_link矩阵是全局坐标系下面的车辆相对于全局坐标原点的转换矩阵，可以表示车辆坐标系原点在全局坐标系的位置
    //t_localizer矩阵是全局坐标系下面的激光雷达相对于全局坐标系的转换矩阵，可以表示激光雷达在全局坐标系的位置
...
int main(int argc, char **argv) {
    ...
    //机器人坐标系的全局坐标系是base，某一个机器人的部件的局部坐标系是link，一个机器人有很多个局部坐标系。
